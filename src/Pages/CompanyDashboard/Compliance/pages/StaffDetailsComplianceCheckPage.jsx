...
const StaffDetailsComplianceCheckPage = () => {
  ...
  // Helper to detect if item is a text submission (based on populated metadata)
  ...
  // Helper function to determine screening score display
  const getScreeningScoreDisplay = (applicant) => {
    const hasScreeningScore =
      applicant?.screening_score !== null &&
      applicant?.screening_score !== undefined;

    const screeningScore = hasScreeningScore
      ? `${applicant.screening_score}%`
      : null;

    let manualShortlistDisplay = null;

    if (applicant?.status_history && Array.isArray(applicant.status_history)) {
      const shortlistHistory = applicant.status_history.find(
        (history) => history.status === "shortlisted" && !history.automated
      );

      if (shortlistHistory) {
        const changerName = shortlistHistory.changed_by?.name || "Unknown User";
        const timestamp = new Date(
          shortlistHistory.timestamp
        ).toLocaleDateString();
        const reason = shortlistHistory.reason || "Manual shortlist";

        manualShortlistDisplay = (
          <div style={{ fontSize: "0.875rem", lineHeight: "1.4" }}>
            <div style={{ fontWeight: "500", color: "#28a745" }}>
              Manual Shortlist by {changerName}
            </div>
    ...
    // Combine both parts (if available)
    return (
      <div>
        {screeningScore && <div>{screeningScore}</div>}
        {manualShortlistDisplay || (!screeningScore && "N/A")}
      </div>
    );
  };
  ...
  const complianceColumns = [
    ...
    },
    {
      ...
      header: "Status",
      render: (item) => {
        // Show "Submitted" only for pre-accepted text items; otherwise use actual status
        const displayStatus =
          isTextItem(item) && item.status === "submitted"
            ? "Submitted"
            : item.status || "pending";
  ...
  const uploadedRequired = complianceChecklist.filter(
    (item) => item.required && (hasFileUpload(item) || hasTextSubmission(item))
  ).length;
            ...
            <table className="info-table">
              <tbody>
                ...
                <tr>
                  ...
                  <td>{getScreeningScoreDisplay(applicant)}</td>
                  {/* <td>
