...
const PersonalInfoStep = ({
  ...
  const [emailType, setEmailType] = useState("tenant");
  ...
  const phoneCodeResult = usePhoneCountryCode();
  const phoneCodeOptions = phoneCodeResult.options;
  const phoneCodeLoading = phoneCodeResult.loading;
  ...
  const emailTypeOptions = [
    { value: "tenant", label: `${tenantName || "Tenant"}    Email` },
    { value: "custom", label: "Custom Email" },
  ];

  // Get tenant domain and name from localStorage
  useEffect(() => {
    const domain = localStorage.getItem("tenantDomain");
    if (domain) {
      setTenantDomain(domain);
    }
    const name = localStorage.getItem("tenantDomain");
    if (name) {
      setTenantName(name);
    }
  }, []);
  ...
  useEffect(() => {
    if (emailType === "tenant" && tenantDomain && formData.workEmail) {
  ...
  }, [tenantDomain, formData.workEmail, emailType]);
  ...
  const renderEmailInput = (isLogin = false) => {
    const label = isLogin ? "Login Email" : "Work Email";
    return (
