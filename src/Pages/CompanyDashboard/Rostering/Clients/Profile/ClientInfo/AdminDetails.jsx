...
const AdminDetails = ({ onEdit, data = {} }) => {
  ...
  const getArrayDisplay = (arr) =>
    arr?.length > 0 ? arr.join(", ") : "Not specified";
    ...
    <div className="Info-Palt">
      ...
      <div className="Info-Palt-Main No-Grid">
        <div className="Info-TTb-BS-HYH">
          ...
          <div className="JUH-PART">
            ...
            <h6>
              <span
                className={`status ${data.currentStatus?.toLowerCase() || ""}`}
              >
                {getDisplayValue(data.currentStatus)}
              </span>
            </h6>
          </div>
        </div>
      ...
      <div className="Info-Palt-Main No-Grid">
        <div className="Info-TTb-BS-HYH">
          ...
          <div className="JUH-PART">
            <p>
              Assign an overall risk level to client in line with your
              contingency plan
            </p>
            <h6>
              <span className={`status ${data.riskLevel?.toLowerCase() || ""}`}>
                {getDisplayValue(data.riskLevel)}
              </span>
            </h6>
          </div>
          <div className="JUH-PART">
            <p>Risk level details</p>
            {data.riskDetails?.map((item) => <h4 key={item}>{item}</h4>) || (
              <h4>Not specified</h4>
            )}
          </div>
        </div>
      ...
      <div className="Info-Palt-Main No-Grid">
        <div className="Info-TTb-BS-HYH">
          <h3>
            What is the contingency plan for client care, in the case of a
            staffing crisis?
          </h3>
          <div className="JUH-PART">
