...
const MaterialRequisitions = () => {
  ...
  const getNotes = (row) => {
    return row.description || row.reason_for_request || "No notes";
  };
  ...
  const getNumericId = (reqId) => {
    const match = reqId ? reqId.match(/(\d{4})$/) : null;
  ...
  const loadData = async () => {
    try {
      ...
      if (Array.isArray(data)) {
        const materialRequests = data.filter(
          (r) => r.request_type === "material"
        );
        setRequests(materialRequests);
        setFilteredData(materialRequests);
      } else {
        setRequests([]);
      }
    } catch (err) {
      ...
      data = data.filter(
        (row) =>
          ...
          (row.reason_for_request &&
            row.reason_for_request.toLowerCase().includes(query))
      );
    }
  ...
  if (loading) {
    return <LoadingState text="Loading Material Requests" />;
  }
                ...
                {currentRows.length > 0 ? (
                  currentRows.map((row, index) => (
                    <tr key={index}>
                      ...
                      <td>{capitalizeFirstLetter(row?.material_type)}</td>
