...
import { createPublicExternalRequest } from "./apiConfig";
...
const CreatePublicExternalRequestPage = () => {
  const params = useParams();
  const navigate = useNavigate();
  const encodedTenantId = params.tenantId;
  if (!encodedTenantId) {
    return <EmptyState message="Error occurred!" />;
  }
  const tenantId = deobfuscate(encodedTenantId);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await createPublicExternalRequest(submitData);
      setSuccessMessage(
...
export default CreatePublicExternalRequestPage;
