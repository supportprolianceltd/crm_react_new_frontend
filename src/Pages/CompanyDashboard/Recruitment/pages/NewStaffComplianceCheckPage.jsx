...
const NewStaffComplianceCheckPage = () => {
  ...
  const statistics = useMemo(() => {
    ...
    // Assuming fields exist; adjust based on actual data structure
    const missingDocs = hiredApplications.filter(
      (app) => (app.missing_documents || 0) > 0
    ).length;
    const respondedCount =
      hiredApplications.filter((app) => app.responded).length || totalNewStaff; // Assume all responded if no field
    const passedCount = hiredApplications.filter(
      (app) => app.compliance_status === "passed"
    ).length;
    const rejectedCount = hiredApplications.filter(
      (app) => app.compliance_status === "rejected"
    ).length;
  ...
  const fetchAllHiredApplications = useCallback(async () => {
    ...
    try {
      ...
      const requisitionsResponse = await fetchAllRequisitions();
      const allApplications = await Promise.all(
        requisitionsResponse?.results?.map(async (req) => {
          const appsResponse = await fetchJobApplicationsByRequisition(req.id);
          return appsResponse?.results
            ...
            .map((app) => ({
              ...
              // Handle compliance_status: it's an array, default to 'pending' if empty
              compliance_status:
                app.compliance_status?.length > 0
                  ? app.compliance_status[0]?.status || "pending"
                  : "pending",
              // missing_documents: derive from compliance_status array (count pending required items)
              missing_documents: app.compliance_status
                ? app.compliance_status.filter((cs) => cs.status === "pending")
                    .length
                : 0,
              // responded: assume true for hired
              responded: true,
  ...
  const tableColumns = useMemo(
    () => [
      ...
      },
      {
        ...
        render: (app) => (
          <StatusBadge status={app.compliance_status || "pending"} />
        ),
      },
      {
        key: "actions",
      ...
      if (filterType === "compliance" && selectedValues.length > 0) {
        filtered = filtered.filter((app) =>
          selectedValues.includes(
            (app.compliance_status || "pending").toLowerCase()
          )
        );
      }
    ...
    (action, app, e) => {
      ...
      switch (action) {
        case "check-staff-compliance":
          navigate(`/company/audit-compliance/recruitment/new-staff/${app.id}`);
          break;
        default:
          break;
      }
    ...
    >
      ...
      <div className="compliance-list-table">
        <Table
          ...
          onRowClick={(id) =>
            navigate(`/company/audit-compliance/recruitment/new-staff/${id}`)
          }
