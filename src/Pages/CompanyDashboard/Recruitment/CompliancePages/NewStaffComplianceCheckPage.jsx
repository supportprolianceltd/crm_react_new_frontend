...
const NewStaffComplianceCheckPage = () => {
  ...
  const getOverallComplianceStatus = useCallback((app) => {
    if (!app.compliance_status || app.compliance_status.length === 0)
      return "pending";
    const hasMissing = app.compliance_status.some(
      (cs) =>
        !cs.name.toLowerCase().includes("reference") &&
        cs.requires_document &&
        cs.status !== "uploaded"
    );
    return hasMissing ? "pending" : "passed";
  }, []);
  ...
  const statistics = useMemo(() => {
    ...
    const missingDocs = hiredApplications.filter((app) => {
      if (!app.compliance_status || app.compliance_status.length === 0)
        return true;
      return app.compliance_status.some(
        (cs) =>
          !cs.name.toLowerCase().includes("reference") &&
          cs.requires_document &&
          cs.status !== "uploaded"
    ...
    const respondedCount = hiredApplications.filter(
      (app) =>
        ...
        app.compliance_status.some(
          (cs) => cs.document && cs.document.length > 1
        )
    ).length;
    ...
    (action, app, e) => {
      ...
      switch (action) {
        case "check-staff-compliance":
          navigate(`/company/recruitment/compliance/new-staff/${id}`);
          break;
        default:
          break;
      }
