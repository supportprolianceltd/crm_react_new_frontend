...
import { fetchAllClusters, updateCluster, deleteCluster, fetchClusterClients, fetchClusterCarers } from '../Cluster/config/apiConfig';

import ClusTImg from './Img/location-icon.png';

// const initialClusters = [

//   { id: 1, name: "Aggrey Road, Port Harcourt, Nigeria", region: "Aggrey Road PHC", postcode: "500101", coords: [-0.141563, 51.50101], description: "Cluster for central London area" },
...
//   { id: 10, name: "Hammersmith Cluster ", region: "Hammersmith QWERTY", postcode: "W6 9JU", coords: [-0.234114, 51.491514], description: "Cluster for Hammersmith area" },
// ];
...
const mapApiClientToLocal = (apiClient) => ({
  id: apiClient.id,
  initials: getInitials(`${apiClient.first_name} ${apiClient.last_name}`),
  fullName: `${apiClient.first_name} ${apiClient.last_name}`,
  address: apiClient.profile?.address_line || 'N/A',
  postcode: apiClient.profile?.postcode || 'N/A',
  status: apiClient.profile?.status?.toLowerCase() || 'inactive',
  visitTime: '',
});

const mapApiCarerToLocal = (apiCarer) => ({
  id: apiCarer.id,
  initials: getInitials(apiCarer.name || ''),
  fullName: apiCarer.name || '',
  address: apiCarer.address || 'N/A',
  postcode: apiCarer.postcode || 'N/A',
  status: apiCarer.status || 'inactive',
  shiftTime: '',
});
...
const NewCluster = () => {
  ...
  }, [currentView, allClients, allCarers]);
  ...
  const clusterClientsCount = useMemo(() => allClients.length, [allClients]);
  ...
  const currentClientsCount = useMemo(() => allClients.length, [allClients]);
