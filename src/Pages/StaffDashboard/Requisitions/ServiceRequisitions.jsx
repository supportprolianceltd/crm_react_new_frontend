...
const ServiceRequisitions = () => {
  ...
  const getNotes = (row) => {
    return row.description || "No notes";
  };
  ...
  const loadData = async () => {
    try {
      ...
      if (Array.isArray(data)) {
        const serviceRequests = data.filter(
          (r) => r.request_type === "service"
        );
    ...
    if (searchQuery.trim() !== "") {
      ...
      data = data.filter(
        (row) =>
          ...
          (row.service_type &&
            row.service_type.toLowerCase().includes(query)) ||
          (row.requester_name &&
            row.requester_name.toLowerCase().includes(query)) ||
          (row.department && row.department.toLowerCase().includes(query)) ||
          (row.contact_info &&
            row.contact_info.toLowerCase().includes(query)) ||
          (row.comment && row.comment.toLowerCase().includes(query))
      );
    }
  ...
  if (loading) {
    return <LoadingState text="Loading Service Requests" />;
  }
        ...
        <div className="PPOl-COnt">
          <div className="PPOlaj-SSde-TopSSUB">
            ...
            <div className="oIK-Search">
              ...
              <input
                type="text"
                placeholder="Search by ID, Description, Type, Name, Department..."
                value={searchQuery}
                ...
                {currentRows.length > 0 ? (
                  currentRows.map((row, index) => (
                    <tr key={index}>
                      ...
                      <td>{getField(row, "title")}</td>
                      <td>{normalizeText(getField(row, "service_type"))}</td>
                      <td>{row.service_description}</td>
                      <td>{formatDate(row.created_at)}</td>
                      <td>
                        <StatusBadge status={titleCase(row.priority_level)} />
                      </td>
                      <td>{formatDate(row.desired_completion_date)}</td>
                      <td>
